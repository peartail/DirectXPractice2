...
bool BumpmapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"bumpmap.fx", L"bumpmap.fx");
	if (!result)
	{
		return false;
	}
...
void BumpmapShaderClass::RotationYawPitchRoll(float y, float p, float r)
{
	yaw = y; pitch = p; roll = r;
}

void BumpmapShaderClass::TranslationMatrix(float x, float y, float z)
{
	posx = x; posy = y; posz = z;
}


bool BumpmapShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView** texarr,D3DXVECTOR3 lightdir,D3DXVECTOR4 difcolor)
{
	bool result;
	...
	result = SetShaderParameters(context, world, view, proj, texarr,lightdir,difcolor);
	if (!result)
	{
		return false;
	}
...
bool BumpmapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
...
void BumpmapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errormessage, HWND hwnd, WCHAR* shaderfile)
{
	char* compileError;
...
bool BumpmapShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView** texarr, D3DXVECTOR3 lightdir, D3DXVECTOR4 difcolor)
{
	HRESULT result;
	...
	dataPtr2->diffuseColor = difcolor;
	...
	context->PSSetConstantBuffers(bufferNumber, 1, &_lightBuffer);

	return true;
}

void BumpmapShaderClass::RenderShader(ID3D11DeviceContext* context, int indexCnt)
{
