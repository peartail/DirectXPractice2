...
bool FadeShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"fade.fx", L"fade.fx");
	if (!result)
	{
		return false;
	}
...
bool FadeShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* texture,  float fade)
{
	bool result;

	result = SetShaderParameters(context, world, view, proj, texture, fade);
	if (!result)
	{
		return false;
	}
...
bool FadeShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
	...
	result = device->CreateBuffer(&fadeBufferDesc, NULL, &_fadebuffer);
	if (FAILED(result))
	{
		return false;
	}
...
bool FadeShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* texture,float fade)
{
	HRESULT result;
	...
	result = context->Map(_fadebuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return false;
	}
	...
	dataPtr2->fadeAmount = fade;

	context->Unmap(_fadebuffer, 0);

	bufferNumber = 1;

	context->PSSetConstantBuffers(bufferNumber, 1, &_fadebuffer);
