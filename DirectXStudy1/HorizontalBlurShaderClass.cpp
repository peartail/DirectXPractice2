...
bool HorizontalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L".\\Shader\\horizontalblur.fx", L".\\Shader\\horizontalblur.fx");
	if (!result)
	{
		return false;
	}
...
bool HorizontalBlurShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* tex, float screenwidth)
{
	bool result;

	result = SetShaderParameters(context, world, view, proj, tex, screenwidth);
	if (!result)
	{
		return false;
	}
...
bool HorizontalBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
...
bool HorizontalBlurShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* tex, float screenwidth)
{
	HRESULT result;
	...
	result = context->Map(_screenSizeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return false;
	}
	...
	dataPtr2->screenWidht = screenwidth;
	dataPtr2->padding = D3DXVECTOR3(0,0,0);

	context->Unmap(_screenSizeBuffer, 0);

	bufferNumber = 1;

	context->VSSetConstantBuffers(bufferNumber, 1, &_screenSizeBuffer);
