...
bool ReflectionShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* reflectionTex,D3DXMATRIX refMat)
{
	bool result;

	result = SetShaderParameters(context, world, view, proj, texture, reflectionTex,refMat);
	if (!result)
	{
		return false;
	}
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
...
bool ReflectionShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* refTex,D3DXMATRIX refmat)
{
	HRESULT result;
	...
	dataPtr2->reflectionMatrix = refmat;
	...
	context->PSSetShaderResources(1, 1, &refTex);
