...
bool ClipPlaneShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"clipplane.fx", L"clipplane.fx");
	if (!result)
	{
		return false;
	}
...
bool ClipPlaneShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* texture,D3DXVECTOR4 clipplane)
{
	bool result;

	result = SetShaderParameters(context, world, view, proj, texture, clipplane);
	if (!result)
	{
		return false;
	}
...
bool ClipPlaneShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
...
void ClipPlaneShaderClass::OutputShaderErrorMessage(ID3D10Blob* errormessage, HWND hwnd, WCHAR* shaderfile)
{
	char* compileError;
...
bool ClipPlaneShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* texture,D3DXVECTOR4 clipplane)
{
	HRESULT result;
	...
	context->VSSetConstantBuffers(bufferNumber, 1, &_clipPlaneBuffer);

	return true;
}

void ClipPlaneShaderClass::RenderShader(ID3D11DeviceContext* context, int indexCnt)
{
