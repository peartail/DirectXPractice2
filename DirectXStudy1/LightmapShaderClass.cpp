...
bool LightmapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"lightmap.fx", L"lightmap.fx");
	if (!result)
	{
		return false;
	}
...
bool LightmapShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView** texarr)
{
	bool result;
...
bool LightmapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
...
void LightmapShaderClass::OutputShaderErrorMessage(ID3D10Blob* errormessage, HWND hwnd, WCHAR* shaderfile)
{
	char* compileError;
...
bool LightmapShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView** texarr)
{
	HRESULT result;
...
void LightmapShaderClass::RenderShader(ID3D11DeviceContext* context, int indexCnt)
{
