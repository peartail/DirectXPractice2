...
bool VerticalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L".\\Shader\\verticalblur.fx", L".\\Shader\\verticalblur.fx");
	if (!result)
	{
		return false;
	}
...
bool VerticalBlurShaderClass::Render(ID3D11DeviceContext* context, int indexCount, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* tex, float screenwidth)
{
	bool result;
...
bool VerticalBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsfile, WCHAR* psfile)
{
	HRESULT result;
...
bool VerticalBlurShaderClass::SetShaderParameters(ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView* tex, float screenwidth)
{
	HRESULT result;
	...
	dataPtr2->screenHeight = screenwidth;
